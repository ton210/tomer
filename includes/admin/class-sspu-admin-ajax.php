...
/**
     ...
     */
    /**
 * Detect color using AI (Enhanced version with imaginative colors)
 */
public function handle_detect_color()
{
    ...
    $messages = [
        [
            'role' => 'system',
            'content' => 'You are a color identification expert with extensive knowledge of color names used in fashion, design, and e-commerce. Your task is to identify the primary color of a product in an image and return ONLY the color name.

Rules:
1. Use descriptive, creative color names when appropriate (e.g., "Charcoal" instead of "Dark Gray", "Teal" instead of "Blue-Green", "Crimson" instead of "Red")
2. For common colors, use enhanced descriptors when the shade is distinctive (e.g., "Navy Blue", "Forest Green", "Rose Gold")
3. Return ONLY the color name in 1-2 words maximum
4. No punctuation, no explanation, just the color name
5. Consider these examples:
   - Light pink → "Blush"
   - Dark blue → "Navy" or "Midnight Blue"
   - Bright pink → "Hot Pink" or "Fuchsia"
   - Gray → "Charcoal", "Ash", "Steel", or "Slate" depending on shade
   - Light blue → "Sky Blue", "Powder Blue", or "Ice Blue"
   - Purple → "Lavender", "Plum", "Violet", or "Amethyst"
   - Brown → "Chocolate", "Mocha", "Tan", "Caramel", or "Espresso"
   - Orange → "Coral", "Tangerine", "Peach", or "Sunset"
   - Green → "Emerald", "Sage", "Mint", "Olive", or "Forest"
6. Always prefer the more specific, imaginative name over generic colors'
        ],
        [
            'role' => 'user',
            'content' => [
                [
                    'type' => 'text',
                    'text' => 'What is the primary color of this product? Return only the color name.'
                ],
                [
                    'type' => 'image_url',
    ...
    $response = wp_remote_post('https://api.openai.com/v1/chat/completions', [
        ...
        'body' => json_encode([
            'model' => 'gpt-4o-mini', // Changed to gpt-4o-mini for cost efficiency
            'messages' => $messages,
            'max_tokens' => 10,
    ...
    if (isset($body['choices'][0]['message']['content'])) {
        ...
        if (!empty($color)) {
            ...
            wp_send_json_error(['message' => 'Could not determine color from image.']);
        }
    } else {
